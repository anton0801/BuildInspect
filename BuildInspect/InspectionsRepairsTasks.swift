...
extension WebCoordinator: WKNavigationDelegate {
    ...
    func webView(_ webView: WKWebView, didCommit navigation: WKNavigation!) {
        if let current = webView.url { checkpoint = current; print("✅ [BuildInspect] Commit: \(current.absoluteString)") }
    }
...
final class WebCoordinator: NSObject {
    ...
    private let cookieJar = "buildinspect_cookies"
    
    func loadURL(_ url: URL, in webView: WKWebView) {
        print("🏗️ [BuildInspect] Load: \(url.absoluteString)")
        redirectCount = 0
